...
document.getElementById('instance-label').innerHTML = `Agent <code>${instanceId}</code>`;
...
// The server prepends a publishing brief to the first turn of each session. It is app
// plumbing, not conversation: hide it when rendering stored history.
const BRIEF_END = 'End of app context. The user message follows.';
function stripBrief(text) {
  if (!text?.startsWith('App context (from the site-builder app')) return text;
  const cut = text.indexOf(BRIEF_END);
  return cut === -1 ? text : text.slice(cut + BRIEF_END.length).replace(/^\n+/, '');
}
  ...
  const detailed = await Promise.all(
    data.slice(0, 30).map(async (session) => {
      ...
      try {
        ...
        return { ...session, title: first ? stripBrief(first.content).slice(0, 60) : 'New session' };
      } catch {
...
async function openSession(id) {
  ...
  for (const message of history) {
    ...
    bubble.setText(message.role === 'user' ? stripBrief(message.content) : message.content);
  }
  loadSessions();
  scrollToBottom();
}
...
function showStarters(show) {
  startersEl.hidden = !show;
}

for (const button of startersEl.querySelectorAll('[data-starter]')) {
  button.addEventListener('click', () => {
    if (sendBtn.hidden) return; // a turn is running
    sendTurn(button.textContent.trim());
  });
}
...
async function sendTurn(text) {
  ...
  try {
    response = await api.stream('/responses', {
      method: 'POST',
      body: JSON.stringify({ input: text, stream: true, ...(sessionId ? { session_id: sessionId } : {}) }),
    });
  } catch (err) {
    setBusy(false);
    bubble.remove();
  ...
  loadSiteLink(); // the turn may have just published the site
}
